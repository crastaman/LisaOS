...
1. Every governed action must yield an append-only, attributed evidence record
   referencing the grant it acted under. **No evidence = ungoverned.** On the
   governed dispatcher path, successful graph completion may be finalized only
   after the evidence append succeeds. If the sink is unavailable, the action
   is not represented as successfully complete and Article VII.2(d) requires a
   systemic halt; the Constitution does not claim physical persistence is
   guaranteed through disk, filesystem, process, or hardware failure.
...
1. **Fail-closed principle.** Where authority, precedence, or constitutional
   ...
   governed work does not proceed. Failure is recorded whenever the evidence
   mechanism remains writable; if it is not writable, that inability is itself
   a systemic halt condition and may only be surfaced through the remaining
   in-memory/error channel. Nothing is treated as successful merely because
