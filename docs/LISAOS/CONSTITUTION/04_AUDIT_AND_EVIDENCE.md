...
appears only in `tests/test_anti_regression.py`, as a **test fixture value and
assertion** — it does not appear in the `WorkAssignment` field comment and is
not an emitted runtime value; and `core/anti_regression.py` classifies by the
