...
| **T15** | **Attribution falsification** — an executor performing work in the orchestrator's own process while the evidence records worker execution | Execution attribution must be sourced from a declaration, never assumed (`04` §1.7); the orchestrator may never execute governed work (`01` §2) | **Partial — declaration-based** (r4). Executors must declare `worker-real` / `worker-simulated` / `main-inline`; the dispatcher fails closed on an undeclared executor, records the declaration, and derives worker/main attribution from it; wrappers inherit and cannot invent a declaration. **But a declaration is not a proof** — a deliberately mismarked in-process callable is not detected. Before r4 this was worse than norm-only: `by_main=False` was hardcoded, so the substrate actively manufactured false attribution | Medium — deliberate misdeclaration is undetected. Common accidental laundering routes are closed (undeclared executors, unmarked wrappers, `ledger_recording_executor` around an undeclared inner, and post-construction executor replacement all fail closed), **but one accidental route remains open**: a `functools.wraps` wrapper that *replaces* rather than *delegates to* a marked executor inherits that executor's provenance declaration, because `functools.wraps` copies `__dict__`. Verified 2026-07-31. This route is **not fixed** in r5 — it is recorded, not remediated |
...
evidence *on the governed path*. Two things it genuinely prevents:
simulation-by-omission, and — since r4 — *silent* attribution laundering, since
an undeclared executor is now refused outright.
