...
| r8 candidate | **Pre-commit candidate state:** uncommitted documentation-only self-description and evidence-state remediation following the r7 final review. No r8 commit or tag is claimed. | This index and the three exact recovered review artifacts listed below; uniform r8 proposal metadata; later audit-chain annotations in `V2_R6_REMEDIATION_EVIDENCE.md`; the permitted NB1 factual-locus correction. | Corrects r8 self-identification and immutable evidence-state records only. It introduces no constitutional semantic, governance, implementation, architecture, test, threat-model, or residual-risk change and does not ratify the proposal. |
